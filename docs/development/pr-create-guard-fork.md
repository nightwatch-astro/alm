...
## Bead-linkage validation, deleted upstream
...
| 3 | `advise()`, line 73 | New. Emits a `PreToolUse` allow decision whose text names the check that could not reach a verdict. Upstream has only `deny()`. |
...
| 10 | `bead_record()`, line 437 | Rewritten. Retries once at `timeout=30` per attempt, against upstream's single attempt at `timeout=5`. Keeps stderr instead of discarding it. Raises `BeadUnavailable` when `bd` is missing, times out, returns unparsable JSON, or fails with a message `MISS_MESSAGE` does not match. Unwraps a single-element list payload. Reads a payload whose `error` key is set as a miss. |
...
| 13 | `__main__` block, line 633 | Wraps `main()` so any `BaseException` other than `SystemExit` exits 0. Without the wrapper, a payload whose `cwd` is not a string raises `TypeError` and exits 1. |

## The lock pin, and why it stays put
...
Do not rewrite those hashes to match the fork.
...
The lockfile format defines no fork, override, or patch field
...
To keep the newly deployed script instead, re-apply the divergences above to it
and copy the result to both paths.
