...
| `crates/calibration/master-detect/src/pixinsight.rs:49` `PixInsightDetector::detect` | `stack_count_evidence: false` unconditionally; `is_master` from naming only | a `STACKCNT`/`NCOMBINE` count is header evidence and decides `is_master` for the files this detector actually claims |
...
| A detector reports header evidence only where its own signature claims the file. A count is an input both detectors see, so reporting it unconditionally reduces `detect_master`'s arbitration to registry order. | `PixInsightDetector::detect` |
...
| .2.35 | LIVE | **Yes.** A PixInsight/WBPP file whose `IMAGETYP` was absent reported `stack_count_evidence: false` with `STACKCNT` present in the same input, and `detect_master` uses that flag to arbitrate between detectors, so a header fact was shadowed by a naming guess | `pixinsight.rs:63-66` decides `is_master` from `input.stack_count` and reports the evidence, but only where `owns_header_count` holds: the signature is in `IMAGETYP`, or `IMAGETYP` is absent and the path carries it. A plain base `IMAGETYP` beside a count is Siril's convention and `SirilDetector` keeps it, however the file is named |
...
| `crates/calibration/master-detect/src/pixinsight.rs` | Modify: `stack_count` decides `is_master` and is reported as evidence for files this detector claims; module doc corrected |
...
- No detector change alters which detector `detect_master` credits for a file
  that detector does not claim. `crates/app/inbox/src/scan.rs`
  `realistic_headers_cover_master_permutations_through_scan_and_classify` pins
  the per-convention `detector` labels end to end and is not edited.
...
- [x] `PixInsightDetector` reports and obeys a present `STACKCNT`/`NCOMBINE` for
      the files its own signature claims, leaving Siril's convention to Siril
...
- [x] Disclose every case that is green both ways rather than dropping it:
      ...
      `pixinsight::tests::a_present_stack_count_decides_a_pixinsight_master_on_its_own`.
      `a_multi_kind_status_is_no_match_when_empty` (`ranking.rs:410`) is vacuous
      for `.2.25` and kept as boundary coverage only.
- [x] `cargo test` green for the five touched crates plus `app_core_inbox`
- [x] `.2.35` narrowed after review round 1: reporting the count unconditionally
      relabelled every Siril-convention master as PixInsight's and turned
      `scan.rs` red. `owns_header_count` confines the count to files this
      detector claims, guarded by
      `pixinsight::tests::a_plain_imagetyp_beside_a_count_is_not_this_detector_s_evidence`
...
- [x] Every LIVE finding has a test that fails with the fix reverted
      ...
      `detect_master` plus 1/1 narrowing guard, .2.25 2/3, .13.28 3/3 red;
      .13.37 proven by the inverted route)
- [x] Both separator spellings of the `.2.30` fixtures are asserted with no
      `cfg` gate
- [x] The five touched crates test green, and `app_core_inbox` is green with
      `scan.rs` unmodified
